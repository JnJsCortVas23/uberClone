...
Run these commands in order, or install everything from `package.json` with `npm install`:
...
> ⚠️ This file is **not in the repository** for security reasons. The app will not build correctly without it.

### Firestore rules (trips & history)

Allow authenticated users to read and write their own trips:
...
> ⚠️ Without a valid API key, the map and destination search will not work.
...
3. Get your test credentials (Access Token and Public Key starting with `TEST-`)
...
Users can also pay **in cash** to the driver. No WebView is required; the trip is updated in Firestore with `paymentMethod: 'cash'`.
...
> ⚠️ **Do not push directly to `main`**. Push your changes to your feature branch.
...
- [x] Trip history (Firestore, sorted client-side)
